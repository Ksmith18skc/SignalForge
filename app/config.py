...
class Settings(BaseSettings):
    ...
    # Wallet enrichment + trade fetch is network-bound (one Falcon/Polymarket
    # round-trip per wallet). Run wallets concurrently with a per-call timeout
    # so the scan finishes in seconds instead of stalling on a hung provider.
    scan_wallet_concurrency: int = 8
    scan_provider_call_timeout_seconds: float = 8.0
    # How far back (hours) the signal engine looks for trades to evaluate.
    # Prediction-market wallets open positions well before an event, so a 24h
    # window silently drops valid same-card positions opened the prior evening.
